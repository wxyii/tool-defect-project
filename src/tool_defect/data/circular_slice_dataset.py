"""Generate overlapping circular slices from annular datasets.

Boundary-normalized polar images use circular indexing along their angular
axis.  Adaptive-annular Cartesian images use full-size angular sector masks.
Both generators derive classification labels from the processed mask: a patch
is defective exactly when its binary mask contains at least
``min_foreground_pixels`` foreground pixels.
"""
...
def annular_sector_mask(
    ...
    """Return a full-size periodic angular sector mask.

    Angles follow the same image-coordinate convention as the ring geometry:
    zero degrees points right and positive angles increase downwards.  The
    interval is half-open, ``[start, start + window)``, and wraps at 360
    degrees without introducing a seam gap.
    """

    if len(shape) != 2:
        raise ValueError("shape 必须是二维图像尺寸")
    ...
    relative = np.mod(angles - start_angle_degrees, 360.0)
    return relative < window_degrees
...
def _write_annular_provenance(rows, destination):
    destination = Path(destination)
    destination.parent.mkdir(parents=True, exist_ok=True)
    with destination.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=_ANNULAR_PROVENANCE_FIELDS,
        )
        writer.writeheader()
        writer.writerows(rows)


def build_adaptive_annular_slice_dataset(
    source_data_root,
    source_manifest,
    output_root,
    *,
    ...
    """Build full-size overlapping sectors from adaptive-annular images.

    Adaptive-annular images are Cartesian ``H x W`` images with a centered
    ring.  Every child keeps the same image dimensions and coordinate system;
    pixels outside its periodic angular sector are set to zero in both the
    image and mask.  The parent split is copied to all child sectors.
    """
    ...
    if slice_count < 1:
        raise ValueError("slice_count 必须为正数")
    if not np.isclose(slice_count * float(stride_degrees), 360.0):
        raise ValueError(
            "slice_count * stride_degrees 必须等于 360 度，才能覆盖完整圆周"
        )
    if not 0 < float(window_degrees) <= 360:
        raise ValueError("window_degrees 必须位于 (0, 360] 范围内")
    if float(window_degrees) < float(stride_degrees):
        raise ValueError(
            "window_degrees 必须不小于 stride_degrees，才能形成重叠或无重叠切片"
        )
    ...
    try:
        for index, row in enumerate(source_rows, start=1):
            ...
            try:
                ...
                for patch_index in range(slice_count):
                    start_angle = patch_index * float(stride_degrees)
                    end_angle = start_angle + float(window_degrees)
                    ...
                    provenance_rows.append(
                        {
                            ...
                            "center_x": center_x,
                            "center_y": center_y,
                            "start_angle_degrees": round(start_angle, 6),
                            "end_angle_degrees": round(end_angle, 6),
                            "wraps_seam": str(end_angle > 360.0).lower(),
                            "source_height": current_shape[0],
                            "source_width": current_shape[1],
                            "output_height": int(patch_mask.shape[0]),
                            "output_width": int(patch_mask.shape[1]),
        ...
        report = {
            "status": "failed" if failures else "complete",
            "generator_version": GENERATOR_VERSION,
            "mode": "adaptive-annular-circular-overlap",
            "source_data_root": str(source_data_root),
            "source_manifest": str(source_manifest),
            "output_root": str(output_root),
            "slice_count": slice_count,
            ...
            "stride_degrees": float(stride_degrees),
            "min_foreground_pixels": min_foreground_pixels,
            "center_convention": "(width / 2, height / 2)",
            "input_parent_samples": len(source_rows),
            "generated_samples": len(generated_rows),
            "patches_per_parent": slice_count,
            "source_shape": list(source_shape) if source_shape else None,
