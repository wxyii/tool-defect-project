...
本目录给出从车间采集、中心检测、人工复核到增量训练和模型发布的完整工程化方案。方案以当前仓库的真实能力为起点，并把目标架构中尚未实现的部分明确标为新建内容。
...
| 范围 | 当前情况 | 方案定位 |
|---|---|---|
| 数据 | 180 张刀具图像，98 张合格、82 张不合格；有对应二值掩膜，82 份不合格样本有 Labelme 标注 | **现有复用**，导入数据集版本库 |
| 无泄漏重训练清单 | 排除 2 个冲突样本并做精确去重后保留 172 个样本，训练、验证、测试为 110、28、34 | **现有复用**，作为首个审计基线 |
...
| 训练与评估 | 支持普通训练、两阶段重训练、同测试集比较和配对自助法置信区间 | **现有复用**，接入数据集和模型版本管理 |
| 训练产物 | `outputs/training` 下有 3 组可加载的双任务 `model.json` 与 `weights.h5` | **上线前置**，先校验、评估和登记，再复制到版本化模型仓库 |
...
数据库是业务状态的唯一事实源；对象存储是图片、掩膜、模型和数据集文件的唯一事实源；模型注册表是模型技术版本的唯一事实源。队列只传递任务引用，不保存业务事实或图片内容。
...
| [08-增量训练闭环](08-增量训练闭环.md) | 样本回流、数据集版本、训练、评估、发布和回滚 | 在线检测编排 |
...
`DOC-16` 是已确认整改目标的增量来源，只对其中明确列出的四项变化具有优先级：新增手工批量模式、新写入采用一项一原图、人员业务角色收敛为两类、模型由管理员网页直接上传至隔离区。它不降低 `01` 的安全失败、事实不可变和服务隔离要求，也不允许消费者绕过 `contracts/` 手写网络字段。
