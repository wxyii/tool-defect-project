...
class MetricRegistry:
    ...
    def observe_histogram(
        ...
        *,
        ...
        if observation < 0:
            raise ValueError("直方图观测值不能为负数")
        normalized_buckets = tuple(_finite(bucket) for bucket in buckets)
        if (
            not normalized_buckets
            or any(bucket <= 0 for bucket in normalized_buckets)
            or normalized_buckets != tuple(sorted(set(normalized_buckets)))
        ):
            raise ValueError("直方图桶必须为严格递增的有限正数")
        count_key = self._key(f"{name}_count", labels)
        sum_key = self._key(f"{name}_sum", labels)
        normalized_labels = count_key[1]
        self._points[count_key] = self._points.get(count_key, 0.0) + 1.0
        self._points[sum_key] = (
            self._points.get(sum_key, 0.0) + observation
        )
        for boundary in (*normalized_buckets, math.inf):
            if observation <= boundary:
                boundary_label = (
                    "+Inf" if math.isinf(boundary) else f"{boundary:g}"
                )
                bucket_key = (
                    f"{name}_bucket",
                    tuple(sorted((*normalized_labels, ("le", boundary_label)))),
                )
                self._points[bucket_key] = (
                    self._points.get(bucket_key, 0.0) + 1.0
                )
